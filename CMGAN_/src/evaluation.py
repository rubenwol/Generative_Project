...
def enhance_one_track(model, audio_path, saved_dir, cut_len, n_fft=400, hop=100, save_tracks=False):
    ...
    if save_tracks:
        ...
        saved_path_noisy = os.path.join(saved_dir, 'noisy', name)
        sf.write(saved_path, est_audio, sr)
        sf.write(saved_path_noisy, noisy[0].cpu().numpy(), sr)


    return est_audio, length
    # return est_audio, len(est_audio)
...
def evaluation(model_path, noisy_dir, clean_dir, save_tracks, saved_dir):
    ...
    metrics_avg = metrics_total / (num - error)
    ...
    print('Number of exception:', error)


parser = argparse.ArgumentParser()
parser.add_argument("--model_path", type=str, default='/home/nlp/wolhanr/CMGAN/src/models_disc_saved/models/CMGAN_epoch_84_0.061',
                    help="the path where the model is saved")
parser.add_argument("--test_dir", type=str, default='/home/nlp/wolhanr/NuWave/VCTK-DEMAND/test',
