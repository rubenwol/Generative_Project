...
parser.add_argument("--cut_len", type=int, default=16000*2, help="cut length, default is 2 seconds in denoise "
                                                                 "and dereverberation")
parser.add_argument("--data_dir", type=str, default='/home/nlp/wolhanr/NuWave/VCTK-DEMAND',
                    help="dir of VCTK+DEMAND dataset")
parser.add_argument("--save_model_dir", type=str, default='./models_disc_saved/models',
                    help="dir of saved model")
parser.add_argument("--save_disc_dir", type=str, default='./models_disc_saved/disc',
                    help="dir of saved model")
...
class Trainer:
    def __init__(self, train_ds, test_ds, gpu_id = 0):
        self.n_fft = 400
        self.hop = 100
    ...
    def train(self):
        ...
        for epoch in range(args.epochs):
            ...
            gen_loss = self.test()
            path_model = os.path.join(args.save_model_dir, 'CMGAN_epoch_' + str(epoch) + '_' + str(gen_loss)[:5])
            path_disc = os.path.join(args.save_disc_dir, 'CMGAN_epoch_' + str(epoch) + '_' + str(gen_loss)[:5])
            if not os.path.exists(args.save_model_dir):
                os.makedirs(args.save_model_dir)
            if not os.path.exists(args.save_disc_dir):
                os.makedirs(args.save_disc_dir)
            if self.gpu_id == 0:
                torch.save(self.model.state_dict(), path_model)
                torch.save(self.discriminator.state_dict(), path_disc)
            scheduler_G.step()
            scheduler_D.step()
