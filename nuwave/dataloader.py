...
class DemandDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return clean_ds, noisy_ds, self.clean_wav_name[idx] #, length


def load_data(hparams, ds_dir='/home/nlp/wolhanr/NuWave/VCTK-DEMAND',cut_len=16000*10, s='train'):
    torchaudio.set_audio_backend("sox_io")         # in linux
    dir = os.path.join(ds_dir, s)
    ds = DemandDataset(dir, cut_len)
    dataset = torch.utils.data.DataLoader(dataset=ds, batch_size=hparams.train.batch_size, pin_memory=True, shuffle=False,
                                                drop_last=True, num_workers=hparams.train.num_workers)
